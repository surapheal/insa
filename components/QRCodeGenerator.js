...
import Logo from '../assets/INSA.png'; // Your logo
...
const QRCodeGenerator = () => {
  ...
  const webPageUrl = 'https://surapheal.github.io/nidp/nidp.html';
  ...
  const openGallery = async () => {
    ...
    if (!result.canceled) {
      ...
      handleScanQRCode(result.assets[0].uri);
    }
  };
  ...
  const handleScanQRCode = async (uri) => {
    try {
      ...
      setScannedQRCode(fileContent);
      Alert.alert('QR Code Scanned', `Data: ${fileContent}`);
    } catch (error) {
